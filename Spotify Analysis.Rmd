...
# 
# write.csv(spotifiles, "/Users/ryancarroll/Desktop/Spotify Data/spotifiles.csv") 
...
Note: Spotify Wrapped is released in mid December every year. The total number of minutes listened to from the screenshot earlier in this program shows I listened to 95,897 minutes of music and does not include all of Decembers listening minutes. That is why 2024 total minutes listened to does not exactly line up with the Spotify Wrapped screenshot.

```{r echo = FALSE, warning = FALSE}

table_cleaned_songs <- time_listened_cleaned_years %>%
  filter(skipped == "FALSE") %>%
  select(year, master_metadata_track_name, master_metadata_album_artist_name, master_metadata_album_album_name, minutes_played, date, minutes_played) %>%
  group_by(year, master_metadata_track_name, master_metadata_album_artist_name) %>% 
  mutate(times_listened_total = n(), 
         last_time_listened = max(date), 
         total_minutes_played = round(sum(minutes_played))) %>%
  select("Song Title" = master_metadata_track_name, "Artist Name" = master_metadata_album_artist_name, "Album Name" = master_metadata_album_artist_name, "Total Minutes Played" = total_minutes_played, "Last Time Played" = last_time_listened, "Total Times Played" = times_listened_total, year) %>%
  unique() %>%
  arrange(desc(`Total Times Played`)) %>%
  group_by(year) %>%
  slice_max(`Total Times Played`, n = 100) 
